...
+Projeto: /workspace/teste-intuitive-care
+Autor: (Josué Araújo Saraiva)
+-->
+
+
+Olá! 👋  
+Este repositório contém minha solução para o **Teste Estagiários** da Intuitive Care.  
...
+
+> **Nota:** Para entregar a melhor solução possível, **comprei um curso na Udemy e estudei por 2 dias** as tecnologias envolvidas (Python, FastAPI, Vue 3, processamento de dados e SQL). Isso me permitiu tomar decisões mais conscientes e implementar tudo com qualidade.
+
+---
+
...
+- **Pipeline de dados (Python)**: download, extração, limpeza, consolidação, validação e agregação.
+- **Banco de dados (PostgreSQL)**: modelagem normalizada + scripts de carga + queries analíticas.
+- **Frontend (Vue 3 + Vite)**: lista de operadoras, busca, paginação, gráfico e detalhes.
+
...
│  │  └─ script/           # scripts de carga no Postgres
│  ├─ script/              # pipeline de dados (download → consolidação → agregação)
│  ├─ .env                 # variáveis do banco (DB_HOST, DB_NAME, etc.)
│  └─ requirements.txt     # dependências do Python
├─ frontend/
│  ├─ src/                 # código Vue (pages, router, composables, etc.)
